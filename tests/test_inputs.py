...
from llm_wrapper_cli.inputs import read_inputs, read_file

def test_read_inputs_url():
    with patch('llm_wrapper_cli.inputs.extract_markdown', return_value='Markdown content'):
        result = read_inputs(['https://example.com'])
    assert result == 'Markdown content'

def test_read_inputs_file():
    file_path = '/path/to/file.txt'
    with patch('llm_wrapper_cli.inputs.Path') as MockPath:
