...
def mock_files(monkeypatch):
    ...
    mock_package.iterdir.return_value = [
        MagicMock(stem="test1", read_text=lambda: "content1")
    ]
    monkeypatch.setattr(
        "llm_wrapper_cli.prompts.files", MagicMock(return_value=mock_package)
    )
...
def test_load_prompts_no_user_folder(mock_files, monkeypatch):
    ...
    monkeypatch.setattr("llm_wrapper_cli.prompts.USER_PROMPT_FOLDER", mock_folder)
